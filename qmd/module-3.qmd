...
penguins |>
  filter(
    ...
    bill_length_mm < 40 | !is.na(bill_length_mm)
  )
```
...
The `dplyr` packages provides a dataset called [`starwars`](https://dplyr.tidyverse.org/reference/starwars.html) that contains information about characters from the Star Wars universe. Filter the `starwars` dataset to include only characters who meet the following criteria:
