...
The `clean_names()` function returns [machine-readable](https://en.m.wikipedia.org/wiki/Machine-readable_medium_and_data) names.
...
We need assign proper classes to the variables. We also need to transform the `year` column, extracting just the year value.
...
Let's organize our columns in a more logical order. The `year` column is best placed next to the `sex` column.
