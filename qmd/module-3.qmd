...
**Tip**: For details on identifying biologically implausible values (BIVs) in anthropometric data, see the World Health Organization (WHO) [`anthro`](https://cran.r-project.org/package=anthro) R package.
...
Projected monthly average maximum temperatures (°C) for June 2021–2040, based on the global climate model ([GCM](https://en.wikipedia.org/wiki/Climate_model)) [ACCESS-CM2](https://research.csiro.au/access/about/cm2/).
:::
::::
...
:::
::: {.column style="width: 85%; padding-top: 15px;"}
[`readr`](https://readr.tidyverse.org) provides a fast and easy way to read rectangular data from delimited files, such as Comma-Separated Values ([CSV](https://en.wikipedia.org/wiki/Comma-separated_values)) and Tab-Separated Values ([TSV](https://en.wikipedia.org/wiki/Tab-separated_values)).
...
- [`mutate()`](https://dplyr.tidyverse.org/reference/mutate.html): Create, modify, and delete columns.
- [`transmute()`](https://dplyr.tidyverse.org/reference/transmute.html): Creates a new data frame containing only specified computations
- [`select()`](https://dplyr.tidyverse.org/reference/select.html): Keep or drop columns using their names and types.
- [`slice()`](https://dplyr.tidyverse.org/reference/slice.html): Subset rows using their positions.
:::
::: {.column style="width: 5%;"}
:::
::: {.column style="width: 47.5%; font-size: 0.9em;"}
- [`filter()`](https://dplyr.tidyverse.org/reference/filter.html): Keep rows that match a condition.
- [`arrange()`](https://dplyr.tidyverse.org/reference/arrange.html): Order rows using column values.
...
- [`summarize()`](https://dplyr.tidyverse.org/reference/summarize.html): Summarise each group down to one row.
:::
::::
...
You've learned the basic grammar for data manipulation using [Tidyverse](https://tidyverse.org), but many important tools remain to be covered. The following slides introduce some of them.

I strongly encourage you to take the time to explore the [documentation website](https://tidyverse.org/packages/) of each Tidyverse package to discover more functions that can help you in your data analysis workflow. I guarantee your future self will thank you!
...
## Exercise (`filter`) {.smaller}

The [`dplyr`](https://dplyr.tidyverse.org/index.html) package provides a dataset called [`starwars`](https://dplyr.tidyverse.org/reference/starwars.html) that contains information about characters from the Star Wars universe. Filter the `starwars` dataset to include only characters who meet the following criteria:
...
`tidyr` provides two functions to deal with this kind if issue: [`pivot_longer()`](https://tidyr.tidyverse.org/reference/pivot_longer.html) and [`pivot_wider()`](https://tidyr.tidyverse.org/reference/pivot_wider.html).
...
Perform a pivot to transform the [`table4a`](https://tidyr.tidyverse.org/reference/table1.html) dataset, found in the [`tidyr`](https://tidyr.tidyverse.org/) package, from **wide** to **long** format using the [`pivot_longer()`](https://tidyr.tidyverse.org/reference/pivot_longer.html) function.
