...
Create a scatter plot using the [`mpg`](https://ggplot2.tidyverse.org/reference/mpg.html) dataset (available in the [`ggplot2`](https://ggplot2.tidyverse.org/) package) that visualizes the relationship between `displ` (engine displacement) and `hwy` (highway miles per gallon).

Add a  Color the points based on the `class` of the vehicle.
