...
Since we are not verifying the assumptions, **the results of the statistical tests may not be valid**.
...
$$
\begin{cases}
\text{H}_{0}: \Delta \text{Adjusted} \ \text{R}^{2} < \text{MES} \quad \text{or} \quad \text{F-test is not significant} \ (\alpha \geq 0.05) \\
\text{H}_{a}: \Delta \text{Adjusted} \ \text{R}^{2} \geq \text{MES} \quad \text{and} \quad \text{F-test is significant} \ (\alpha < 0.05)
\end{cases}
$$
:::
