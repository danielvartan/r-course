...
It is created by the same team that developed [Tidyverse](https://www.tidyverse.org/) and is designed to work seamlessly with it.
...
[Critique](https://danielvartan.github.io/mastersthesis/web/qmd/supplementary-material-11.html) of Leocadio-Miguel et al. latitude article [@vartanian2024a].
...
\text{H}_{1}: \text{Cohen's} \ d \geq \text{MES} \\
\end{cases}
$$
:::
::::
...
To ensure [practical significance]{.underline}, we will analyze the difference in means for its effect size, considering a 95% confidence interval. Cohen's benchmark for a medium effect size ($d$ = 0.5) will be used as our Minimum Effect Size (MES) [@cohen1988a].
...
A power analysis for a t-test with an expected effect size of **0.5** ($\text{Cohen's} \ d$) indicates that we would need approximately [64]{.brand-triad-blue-red .bold-black} participants in each group to achieve a power of **0.8** at a significance level of **0.05**.
...
::: {style="padding-top: 15px;"}
⚖️ Homogeneity of variances between groups (only if using Student's t-test; [Welch's t-test](https://en.wikipedia.org/wiki/Welch%27s_t-test) and our permutation approach do not require this assumption).
:::
