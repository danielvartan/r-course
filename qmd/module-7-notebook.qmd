...
- Consider Cohen's benchmark (Cohen, 1988) for small effects as the Minimum Effect Size (MES) for practical significance ($d = 0.2$, r = 0.1, $q = 0.1$, g = $0.05$, $h = 0.2$, $w = 0.1$, $f = 0.1$, f^2 = 0.02, $\text{R}^2 \approx 0.0196$).
...
The data used in this analysis were sourced from:

- The Brazilian Institute of Geography and Statistics ([IBGE](https://www.ibge.gov.br/)), for data on the Brazilian territory, accessed using the Institute for Applied Economic Research (IPEA) [`geobr`](https://ipeagit.github.io/geobr/) R package (Pereira & Goncalves, n.d.).
- Brazil's Food and Nutrition Surveillance System ([SISVAN](https://sisaps.saude.gov.br/sisvan/)), for data on ultra-processed food consumption (Sistema de Vigilância Alimentar e Nutricional, n.d.).
...
Source: IBGE data on the Brazilian territory, accessed using IPEA'[`geobr`](https://ipeagit.github.io/geobr/) R package (Pereira & Goncalves, n.d.).
...
file <- here::here("data", "sisvan-tidy.rds")

if (checkmate::test_file_exists(file)) {
  ibge_table_5938 <- readRDS(file)
} else {
...
sisvan_data <-
  ...
  dplyr::rename(
    ...
    state_code = cod_uf,
    state_name = uf,
    ultra_processed_food_consumption = consumo_ultraprocessados
  )
```
