...
[Positron](https://positron.posit.co/) is *flavour* of Visual Studio Code, the most used IDE for programming in any language. It supersedes [RStudio](https://posit.co/download/rstudio-desktop/), the main IDE for R for many years, focusing in providing a better experience for programming in R and Python.
...
::::: {.columns}
:::: {.column style="width: 50%;"}
Atomic types are **non-recursive objects**, i.e., objects that can't hold themselves as an entry (e.g., `logical`, `integer`, `double`, `character`).
